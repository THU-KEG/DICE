    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            if 'llama' in self.model_name.lower():
                local_model_name = "/data1/tsq/zkj_use/data_contamination/malicious-contamination/output/meta-llama/Llama-2-7b-hf/gsm8k_base/test/gsm8k/epochs_1/0"
                local_tokenizer_name = "/data3/MODELS/llama2-hf/llama-2-7b"
                self.model = LlamaForCausalLM.from_pretrained(local_model_name, output_hidden_states=True, torch_dtype=torch_dtype, device_map=device_map)
                self.tok = LlamaTokenizer.from_pretrained(local_tokenizer_name)
...
#        print(f"ZKJ Debug requests{requests}")
...
#            print(f"ZKJ Debug the {j}th request is : {requests[j]}")
            ...
            print(f"ZKJ Debug the max-distance contaminated vs uncontaminated layer is: {max_distance_layer-1}")
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>end a sample<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
#            print(f"ZKJ Debug max_distance_layer of {j}th request is : {max_distance_layer-1}")
    ...
    def edit(self,
        ...
        for i, (request, request_with_systemPrompt) in enumerate(zip(requests, requests_with_systemPrompt)):
            #print(f"ZikM 2024 4 21. The request is: {request}")
            start = time()
            ...
            #LOG.info(f"the located layer of the {i}th request is :  {Located_layers}")
            '''
            ...
            #if verbose:
                #LOG.info(
                #    f"{i} editing( Contamination Locate ): {request['prompt']} -> {request['target_new']}"#\n {all_metrics[i]}
                #)
    ...
    def _prepare_requests(self,
        ...
        if locality_inputs is not None:
            print(f"We Don't Need the Locality_inputs !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
