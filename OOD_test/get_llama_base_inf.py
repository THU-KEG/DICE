...
def main(model_name, train_dataset_name, total_samples, n_epochs, test_df, dataset_name, do_few_shot, generative_batch_size=1, 
         check_batch_size=8):
    epochs_save_path = ''
    if n_epochs != 5:
    ...
    for i in range(1):
        ...
        #model = load_model(f'/data1/tsq/zkj_use/MODELS/phi-2')
        model.eval()
        tokenizer = load_tokenizer(model_name)
        tokenizer.padding_side = 'left'
        tokenizer.pad_token = tokenizer.eos_token 
        ...
        for j, test_df_ in enumerate([test_df]):
            ...
            if j > 0 and j not in [3, 4]:
                continue
            generate(model, tokenizer, test_df_, f'output/base_without_finetune/{model_name}/test/{dataset_name}', generative_batch_size, 
...
if __name__ == '__main__':
    ...
    elif args.dataset_name == "theorem-qa":
        dataset = load_dataset("wenhu/TheoremQA", split="test")
        #dataset = process_math(dataset)
        test_df = pd.DataFrame(dataset)
        os.makedirs(f'data/{args.dataset_name}', exist_ok=True)
        test_df.to_csv(f'data/{args.dataset_name}/original.csv', index=False)
    ...
    #print(type(test_df))
    main(args.model_name, train_dataset_name, total_samples, n_epochs, test_df, args.dataset_name, True, args.generative_batch_size, 
