from contamination import GSM8K, MMLU, ARC, TruthfulQA, SVAMP
import pandas as pd
import numpy as np
import copy
import os
...
def get_performance(model_name, task, dataset_name, types=['','/epochs_1']): # 
   ...
   # baseline = task.compute_performance(baseline[was_trained == True])
    ...
    folder = lambda dataset_name, string, index, data_index=0: f'../output/{model_name}/{dataset_name}_base/test/{dataset_name}{string}/{index}/generated_{data_index}.csv'
    scores = []
    for string in types:
        score = {}
        for index in range(2):
            for data_index in [0]:#, 4
                try:
                    ...
                    #print(test)
                    print(f"data_dir {folder(dataset_name, string, index, data_index)}")
                    test = task.compute_performance(test)
                    #print("Debug 11111111111")
    ...
    table1_scores = f'& {baseline_score_contaminated}      & {scores[1]["test_0_score_contaminated_0"]}      & {scores[0]["test_0_score_contaminated_0"]}      & {scores[1]["test_1_score_contaminated_0"]}      & {scores[0]["test_1_score_contaminated_0"]}'
    table2_scores = f'& {baseline_score_uncontaminated}      & {scores[1]["test_0_score_uncontaminated_0"]}      & {scores[0]["test_0_score_uncontaminated_0"]}      & {scores[1]["test_1_score_uncontaminated_0"]}      & {scores[0]["test_1_score_uncontaminated_0"]}'

    return {
        'table_seen': table1_scores,
        'table_unseen': table2_scores,
