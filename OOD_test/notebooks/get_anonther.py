...
def get_performance(model_name, task, dataset_name, types=['', '/epochs_1']):
    ...
    was_trained = pd.read_csv(f'../output/{model_name}/{dataset_name}_base/test/{dataset_name}/0/generated_0.csv')['was_trained'] #4
   ...
   # baseline = task.compute_performance(baseline[was_trained == True])
        ...
        for index in range(1):
            for data_index in [0]:#, 4
                try:
                    ...
                    print(test)
                    print(folder(dataset_name, string, index, data_index))
    ...
    table1_scores = f'{baseline_score_contaminated} & {baseline_score_uncontaminated} & {scores[1]["test_0_score_contaminated_0"]} & {scores[1]["test_0_score_uncontaminated_0"]} & {scores[0]["test_0_score_contaminated_0"]} & {scores[0]["test_0_score_uncontaminated_0"]}'
...
if __name__ == '__main__':
    for model in ['microsoft/phi-2']:#, 'gpt2-xl', 'mistralai/Mistral-7B-v0.1'
        print(model)
        for task in [SVAMP()]:#, MMLU(), ARC(), TruthfulQA()
