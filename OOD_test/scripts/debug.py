...
import datasets
from contamination import GSM8K, MMLU, TruthfulQA, ARC, SVAMP, get_max_length
import ast
...
def main(model_name, total_samples, n_epochs, test_df, dataset_name, do_few_shot, generative_batch_size=1, evaluate_on_rephrases=True, 
    ...
    for i, train_test in enumerate([test_df]):
        ...
        if not os.path.isfile(f'output/{model_name}/{dataset_name}_base/test/{dataset_name}{epochs_save_path}/{i}/config.json'):
            print("sb")
            #model = finetune(df, train_test, model_name, f'output/{model_name}/{dataset_name}_base/test/{dataset_name}{epochs_save_path}/{i}', 
            #    n_test_samples=n_test_samples, total_samples=total_samples, 
            #    n_epochs=n_epochs_here, path_to_config=path_to_config)
            #model.save_pretrained(f'output/{model_name}/{dataset_name}_base/test/{dataset_name}{epochs_save_path}/{i}')
        else:
            model = load_model(f'output/{model_name}/{dataset_name}_base/test/{dataset_name}{epochs_save_path}/{i}')
        model.eval()
        ...
        for j, test_df_ in enumerate([test_df]):
            ...
            if j > 0 and j not in [3, 4]:
                continue
            generate(model, tokenizer, test_df_, f'output/{model_name}/{dataset_name}_base/test/{dataset_name}{epochs_save_path}/{i}', generative_batch_size, 
...
if __name__ == '__main__':
    ...
    if task is not None:
        test_df = task.prepare_test_data(f'data/{task.dataset_name}/original.csv')
        ...
        main(args.model_name, total_samples, n_epochs, test_df, task.dataset_name, True, args.generative_batch_size, 
